...
$$ \tilde{Y}\_i = \alpha \tilde{D}\_i + \eta\_i.$$
...
1.  Residualiseeri (Y) kontrollide (X) suhtes:
    ...
    $$ \hat{m}\_Y(X_i) = \hat{\mathbb{E}}$$Y_i \mid X_i$$, \quad rY_i = Y_i - \hat{m}\_Y(X_i).$$

2.  Residualiseeri (D) kontrollide (X) suhtes:

    $$ \hat{m}\_D(X_i) = \hat{\mathbb{E}}$$D_i \mid X_i$$, \quad rD_i = D_i - \hat{m}\_D(X_i).$$

3.  Residualiseeri (Z) kontrollide (X) suhtes:

    $$ \hat{m}\_Z(X_i) = \hat{\mathbb{E}}$$Z_i \mid X_i$$, \quad rZ_i = Z_i - \hat{m}\_Z(X_i).$$
...
3.  Hinda (\alpha) regressioonis (\tilde{Y}\_i = \alpha \tilde{D}\_i + \eta\_i).
...
require(dplyr)      # used for data manipulation and joining
require(glmnet) #glmnet, LASSO 
require(hdm)    # LASSO

```

# LASSO ja mõju hindamine

## Lihtne näide LASSOga prognoosimisel
...
Tee k-kordne ristvalideerimine, et leida optimaalne lambda väärtus.
Olgu k = 5
...
## Näited kasutades paketti hdm

`rlasso` teeb nii Lasso kui post-Lasso.
Lubab nii homoskedastiiseid kui heteroskedastiivseid vigu (default homoscedastic = FALSE)
...
## Partialling out method - jääkliikmete meetod
...
### Topelt selektsiooni meetod (double selection method)
  ...
  (1) Vali Lasso abil kontrollmuutujad x_{ij}, mis ennustavad y_i.
  (2) Vali Lasso abil kontrollmuutujad x_{ij}, mis ennustavad d_i.
  (3) Teosta tavaruutude meetod (OLS), kus y_i regresseeritakse d_i ja kontrollmuutujate ühisosa (!) suhtes, mis valiti sammudes 1 ja 2.
